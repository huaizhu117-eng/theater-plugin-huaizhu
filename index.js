// 槐序戏台 - 第四十一版：修复"独立API"标签页横向溢出屏幕的bug——里面的label/select/input一直没有专门的CSS，用的是浏览器默认行内显示，所以"平台"下拉框和"API地址"标签会紧挨着挤在同一行，select按内容撑开宽度还把整个面板撑出了横向滚动条。现在把这些元素都改成block+100%宽度，每个都独占一行、宽度跟随面板，不会再超出去；另外给密钥输入框的flex子项加了min-width:0，避免长密钥同样把容器撑爆
    ...
    <div id="huaizhu_modal_overlay" class="huaizhu-modal-overlay" style="display:none;">
      <div class="huaizhu-modal-box">
        ...
        <div class="huaizhu-meta-row">
          <span class="huaizhu-meta-text">版本：0.4.1</span>
