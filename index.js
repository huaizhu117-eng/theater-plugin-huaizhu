// 槐序戏台 - 第五十六版：①修复"导入数据"按钮错误——之前点导入会展开一个粘贴文本框（残留的旧实现），但HTML里其实已经有<input type="file">了，JS却没有接上它。现在改成点"导入数据"直接弹出系统文件选择器，选择之前导出的.json备份文件，读取并写入localStorage，不需要手动粘贴；②"查看数据状态"展开后面板变高按钮被推到屏幕外，改成展开后自动滚到顶部确保按钮可见，按钮文字改成"🔼 点击收起"更直观
...
jQuery(async () => {
    ...
    <div class="inline-drawer">
      ...
      <div class="inline-drawer-content">
        ...
        <p class="huaizhu-storage-hint">
          ...
          如果还是觉得卡，可以点上面的按钮手动清空历史记录（不会影响"我的收藏"和"随机生成"）。
        </p>
  ...
  const floatingHtml = `
    ...
    <div id="huaizhu_modal_overlay" class="huaizhu-modal-overlay" style="display:none;">
      <div class="huaizhu-modal-box">
        ...
        <div class="huaizhu-meta-row">
          <span class="huaizhu-meta-text">版本：0.5.6</span>
        ...
        <div class="huaizhu-panel-header">
          <div class="huaizhu-tab-bar">
            ...
            <button class="huaizhu-tab-btn" data-tab="template">随机生成</button>
            <button class="huaizhu-tab-btn" data-tab="preset">预设</button>
            <button class="huaizhu-tab-btn" data-tab="beautify">美化</button>
            <button class="huaizhu-tab-btn" data-tab="settings">独立API</button>
            <button class="huaizhu-tab-btn" data-tab="appsettings">设置</button>
          </div>
        </div>
        ...
        <div id="huaizhu_tab_warehouse" class="huaizhu-tab-page" style="display:none;">
          ...
          <div id="huaizhu_warehouse_list_view">
            ...
            <div class="huaizhu-collapsible-content" style="display:none;">
              <input type="text" id="huaizhu_recent_search_input" placeholder="搜索最近生成...">
              <div id="huaizhu_recent_list_container"></div>
            </div>
            ...
            <div class="huaizhu-collapsible-content">
              ...
              <input type="text" id="huaizhu_favorite_search_input" placeholder="搜索收藏...">
              <div id="huaizhu_favorite_list_container"></div>
            </div>

          </div>

          <!-- 收藏的新建/编辑表单：跟列表共用同一个面板，不再单独弹出一个框叠在上面，
               同样不用panel-header样式，避免看起来像套了一个第二层面板 -->
          <div id="huaizhu_favorite_edit_view" style="display:none;">
            <a href="javascript:void(0)" id="huaizhu_favorite_edit_back" class="huaizhu-edit-back-link">‹ 返回收藏列表</a>
            <h3 class="huaizhu-edit-inline-title">新建收藏</h3>
            ...
            <div class="huaizhu-also-save-template-row">
              <label class="huaizhu-also-save-label">
                <input type="checkbox" id="huaizhu_favorite_also_save_template"> 同时保存到随机生成
              </label>
              <div id="huaizhu_favorite_template_group_row" style="display:none; margin-top:6px;">
                <input type="text" id="huaizhu_favorite_template_group_input" placeholder="分组（选填，用/隔开多个）">
              </div>
            </div>

            <div id="huaizhu_favorite_duplicate_warning" class="huaizhu-duplicate-warning" style="display:none;"></div>
        ...
        <!-- 标签页：随机生成（原"模板"，内部id/函数名仍保留template以减少改动风险，仅展示文案改了名字） -->
        <div id="huaizhu_tab_template" class="huaizhu-tab-page" style="display:none;">

          <div id="huaizhu_template_list_view">

            <button type="button" id="huaizhu_template_add_btn" class="menu_button" style="width:100%; margin:6px 0;">+ 新建随机生成</button>

            <input type="text" id="huaizhu_template_search_input" placeholder="搜索随机生成名称...">
          ...
          <!-- 随机生成的新建/编辑表单：跟列表共用同一个面板，不再单独弹出一个框叠在上面。
               这里特意不用huaizhu-panel-header那种"标题+关闭按钮"的样式，因为那样会看起来像
               面板里又长出了一个小面板的标题栏，跟外层面板自己的标题堆在一起显得很怪；
               改成一行轻量的"返回"链接，视觉上仍然是同一个面板内部的切换，不是叠加的第二层 -->
          <div id="huaizhu_template_edit_view" style="display:none;">
            <a href="javascript:void(0)" id="huaizhu_template_edit_back" class="huaizhu-edit-back-link">‹ 返回随机生成列表</a>
            <h3 id="huaizhu_template_edit_title" class="huaizhu-edit-inline-title">新建随机生成</h3>
            ...
            <input type="text" id="huaizhu_template_group_input" placeholder="例如：美化样式/搞笑（留空则归入未分组）">
        ...
        <!-- 标签页：美化（插件自身界面的外观，不是AI生成内容） -->
        <div id="huaizhu_tab_beautify" class="huaizhu-tab-page" style="display:none;">

          <p style="font-size:0.78em; opacity:0.6; margin:0 0 10px 0;">
            这里美化的是插件自己这个面板（标题栏/标签栏/按钮这些），不影响AI生成的小剧场内容。
          </p>

          <div class="huaizhu-beautify-theme-row">
            <button type="button" class="huaizhu-uitheme-btn huaizhu-style-active" data-uitheme="default">内置默认</button>
            <button type="button" class="huaizhu-uitheme-btn" data-uitheme="tavern">跟随酒馆主题</button>
            <button type="button" class="huaizhu-uitheme-btn" data-uitheme="custom">自定义</button>
          </div>

          <div id="huaizhu_ui_custom_controls" style="display:none;">

            <div class="huaizhu-beautify-section-label">面板背景色</div>
            <div class="huaizhu-color-row">
              <input type="color" id="huaizhu_ui_bg_picker" value="#222222">
              <input type="text" id="huaizhu_ui_bg_text" value="#222222" maxlength="7">
            </div>

            <div class="huaizhu-beautify-section-label">字体颜色</div>
            <div class="huaizhu-color-row">
              <input type="color" id="huaizhu_ui_text_picker" value="#ffffff">
              <input type="text" id="huaizhu_ui_text_text" value="#ffffff" maxlength="7">
            </div>

            <div class="huaizhu-beautify-section-label">强调色（开关/选中标签/边框这些用到的颜色）</div>
            <div class="huaizhu-color-row">
              <input type="color" id="huaizhu_ui_accent_picker" value="#5b4636">
              <input type="text" id="huaizhu_ui_accent_text" value="#5b4636" maxlength="7">
            </div>

            <div class="huaizhu-beautify-section-label">圆角程度</div>
            <div class="huaizhu-slider-row">
              <input type="range" id="huaizhu_ui_radius_slider" min="0" max="24" step="1" value="10">
              <span id="huaizhu_ui_radius_value">10px</span>
            </div>

            <div class="huaizhu-beautify-section-label">字体大小</div>
            <div class="huaizhu-slider-row">
              <input type="range" id="huaizhu_ui_fontscale_slider" min="85" max="130" step="5" value="100">
              <span id="huaizhu_ui_fontscale_value">100%</span>
            </div>

            <div class="huaizhu-beautify-section-label">字体</div>
            <select id="huaizhu_ui_font_select">
              <option value="">跟随酒馆默认</option>
              <option value="'Microsoft Yahei', 'PingFang SC', sans-serif">黑体 / 无衬线</option>
              <option value="'SimSun', 'Songti SC', serif">宋体 / 衬线</option>
              <option value="'KaiTi', 'STKaiti', serif">楷体</option>
              <option value="'Consolas', 'Courier New', monospace">等宽</option>
              <option value="__custom__">自定义字体名称…</option>
            </select>
            <input type="text" id="huaizhu_ui_font_custom_input" placeholder="例如：仓耳渔阳体, sans-serif" style="display:none; margin-top:6px;">

            <div class="huaizhu-beautify-section-title" id="huaizhu_advanced_css_toggle">
              <span>⚙️ 高级：自定义CSS代码（可选，留空则不生效）</span>
              <span class="huaizhu-collapse-arrow huaizhu-collapsed">▾</span>
            </div>
            <div id="huaizhu_advanced_css_content" style="display:none;">
              <p style="font-size:0.75em; opacity:0.55; margin:4px 0 6px 0;">
                面板的选择器主要是 .huaizhu-modal-box / .huaizhu-result-box / .huaizhu-tab-btn 这些，写在这里的代码会直接生效，写错了也不会影响插件本身运行，最多就是没效果。
              </p>
              <textarea id="huaizhu_ui_custom_css_textarea" rows="5" placeholder=".huaizhu-modal-box { ... }"></textarea>
            </div>

            <p style="font-size:0.78em; opacity:0.55; margin:10px 0 0 0;">
              改了立刻就能在这个面板本身上看到效果，不需要单独的预览框。
            </p>
        ...
        <!-- 标签页：设置 -->
        <div id="huaizhu_tab_appsettings" class="huaizhu-tab-page" style="display:none;">

          <div class="huaizhu-switch-row">
            <span>保存时检测相似名称并提示重复</span>
            <label class="huaizhu-switch">
              <input type="checkbox" id="huaizhu_setting_detect_duplicate" checked>
              <span class="huaizhu-switch-slider"></span>
            </label>
          </div>
          <p style="font-size:0.75em; opacity:0.55; margin:4px 0 14px 0;">开启后，在仓库和随机生成里保存新条目前，会自动扫描是否已有名字相似的记录，有的话给出提示（不强制拦截，你自己决定要不要继续保存）</p>

          <div class="huaizhu-beautify-section-label" style="margin-top:18px;">数据备份与恢复</div>
          <p style="font-size:0.75em; opacity:0.55; margin:4px 0 8px 0;">如果插件数据（随机生成、收藏、预设、API设置）看起来丢失，先点「查看数据状态」确认数据是否还在。建议定期导出备份，防止意外丢失。</p>
          <button type="button" id="huaizhu_data_check_btn" class="menu_button" style="width:100%; margin:4px 0;">🔍 查看数据状态</button>
          <div id="huaizhu_data_check_result" style="font-size:0.78em; opacity:0.7; margin:6px 0; display:none;"></div>
          <button type="button" id="huaizhu_data_export_btn" class="menu_button" style="width:100%; margin:4px 0;">📤 导出全部数据（备份）</button>
          <button type="button" id="huaizhu_data_import_btn" class="menu_button" style="width:100%; margin:4px 0;">📥 导入数据（从备份文件恢复）</button>
          <input type="file" id="huaizhu_data_import_file" accept=".json" style="display:none;">
    ...
    <!-- 仓库条目编辑弹窗：最近生成（可编辑名字和内容）和收藏（可编辑名字和内容）共用 -->
    <div id="huaizhu_entry_edit_overlay" class="huaizhu-modal-overlay" style="display:none;">
      <div class="huaizhu-modal-box">
        <div class="huaizhu-panel-header">
          <h3 id="huaizhu_entry_edit_title">编辑</h3>
          <button id="huaizhu_entry_edit_close" class="huaizhu-close-btn">×</button>
        </div>

        <label for="huaizhu_entry_edit_name_input">名称：</label>
        <input type="text" id="huaizhu_entry_edit_name_input" placeholder="">

        <label for="huaizhu_entry_edit_content_input">内容：</label>
        <textarea id="huaizhu_entry_edit_content_input" rows="6" placeholder=""></textarea>

        <div class="huaizhu-modal-buttons">
          <button id="huaizhu_entry_edit_cancel" class="menu_button">取消</button>
          <button id="huaizhu_entry_edit_save" class="menu_button">保存</button>
        </div>
      </div>
    </div>
  ...
  $('#huaizhu_cleanup_btn').on('click', () => {
    if (!confirm('确定要清空"最近生成"的全部历史记录吗？不会影响"我的收藏"和"随机生成"。')) return;
  ...
  // ===================================================================
  // "美化"标签页：自定义插件自身界面的外观（面板背景/字体颜色/强调色/圆角/字体大小），
  // 跟AI生成的小剧场内容完全无关——三种模式：内置默认 / 跟随酒馆主题 / 自定义
  // ===================================================================

  const HUAIZHU_UI_THEME_KEY = 'huaizhu_ui_theme_settings';

  function loadUiThemeSettings() {
    try {
      const raw = localStorage.getItem(HUAIZHU_UI_THEME_KEY);
      const parsed = raw ? JSON.parse(raw) : {};
      return {
        mode: parsed.mode || 'default', // default | tavern | custom
        bgColor: parsed.bgColor || '#222222',
        textColor: parsed.textColor || '#ffffff',
        accentColor: parsed.accentColor || '#5b4636',
        radius: parsed.radius !== undefined ? parsed.radius : 10,
        fontScale: parsed.fontScale !== undefined ? parsed.fontScale : 100,
        fontFamily: parsed.fontFamily || '', // 空字符串='跟随酒馆默认'
        customCss: parsed.customCss || '',
      };
    } catch (e) {
      return {
        mode: 'default', bgColor: '#222222', textColor: '#ffffff', accentColor: '#5b4636',
        radius: 10, fontScale: 100, fontFamily: '', customCss: '',
      };
    }
  }

  function saveUiThemeSettings(settings) {
    localStorage.setItem(HUAIZHU_UI_THEME_KEY, JSON.stringify(settings));
  }

  let uiThemeSettings = loadUiThemeSettings();
  applyUiTheme(); // 页面刚加载/插件刚启用时，立刻把上次保存的主题应用上，不用等用户重新点一下

  // 真正生效的地方：把几个CSS自定义属性设到documentElement上，整个插件面板的CSS全部引用这几个变量，
  // 改这里就能让标题栏/标签栏/按钮这些一起联动变化，不需要逐个元素去改
  function applyUiTheme() {
    const root = document.documentElement.style;

    if (uiThemeSettings.mode === 'tavern') {
      // 引用酒馆自己的主题变量（不是抄一份当前的值，是直接引用），酒馆换主题这里也会跟着变
      root.setProperty('--huaizhu-bg-color', 'var(--SmartThemeBlurTintColor, #222)');
      root.setProperty('--huaizhu-text-color', 'var(--SmartThemeBodyColor, #fff)');
      root.setProperty('--huaizhu-accent-color', 'var(--SmartThemeQuoteColor, #5b4636)');
      root.removeProperty('--huaizhu-radius');
      root.removeProperty('--huaizhu-font-scale');
      root.removeProperty('--huaizhu-font-family');
    } else if (uiThemeSettings.mode === 'custom') {
      root.setProperty('--huaizhu-bg-color', uiThemeSettings.bgColor);
      root.setProperty('--huaizhu-text-color', uiThemeSettings.textColor);
      root.setProperty('--huaizhu-accent-color', uiThemeSettings.accentColor);
      root.setProperty('--huaizhu-radius', uiThemeSettings.radius + 'px');
      root.setProperty('--huaizhu-font-scale', uiThemeSettings.fontScale / 100);
      if (uiThemeSettings.fontFamily) {
        root.setProperty('--huaizhu-font-family', uiThemeSettings.fontFamily);
      } else {
        root.removeProperty('--huaizhu-font-family');
      }
    } else {
      // 内置默认：清空所有覆盖，让CSS文件里var()自带的兜底值生效
      root.removeProperty('--huaizhu-bg-color');
      root.removeProperty('--huaizhu-text-color');
      root.removeProperty('--huaizhu-accent-color');
      root.removeProperty('--huaizhu-radius');
      root.removeProperty('--huaizhu-font-scale');
      root.removeProperty('--huaizhu-font-family');
    }

    // 高级自定义CSS：只在"自定义"模式下才会注入，且留空就不生效；写在单独的<style>标签里，方便整体清空
    let styleTag = document.getElementById('huaizhu_custom_css_override');
    const cssContent = (uiThemeSettings.mode === 'custom' && uiThemeSettings.customCss) ? uiThemeSettings.customCss : '';
    if (cssContent) {
      if (!styleTag) {
        styleTag = document.createElement('style');
        styleTag.id = 'huaizhu_custom_css_override';
  ...
  // 把当前设置同步进表单控件（切到这个标签页时调用）
  function loadUiThemeSettingsIntoForm() {
    $('.huaizhu-uitheme-btn').removeClass('huaizhu-style-active');
    $('.huaizhu-uitheme-btn[data-uitheme="' + uiThemeSettings.mode + '"]').addClass('huaizhu-style-active');
    $('#huaizhu_ui_custom_controls').css('display', uiThemeSettings.mode === 'custom' ? 'block' : 'none');

    $('#huaizhu_ui_bg_picker').val(uiThemeSettings.bgColor);
    $('#huaizhu_ui_bg_text').val(uiThemeSettings.bgColor);
    $('#huaizhu_ui_text_picker').val(uiThemeSettings.textColor);
    $('#huaizhu_ui_text_text').val(uiThemeSettings.textColor);
    $('#huaizhu_ui_accent_picker').val(uiThemeSettings.accentColor);
    $('#huaizhu_ui_accent_text').val(uiThemeSettings.accentColor);
    $('#huaizhu_ui_radius_slider').val(uiThemeSettings.radius);
    $('#huaizhu_ui_radius_value').text(uiThemeSettings.radius + 'px');
    $('#huaizhu_ui_fontscale_slider').val(uiThemeSettings.fontScale);
    $('#huaizhu_ui_fontscale_value').text(uiThemeSettings.fontScale + '%');

    // 字体：判断当前值是不是下拉框里的预设选项之一，不是的话（说明是自定义字体名称）就选中"自定义"并展开输入框
    const fontPresetValues = $('#huaizhu_ui_font_select option').map(function () { return $(this).val(); }).get();
    if (!uiThemeSettings.fontFamily) {
      $('#huaizhu_ui_font_select').val('');
      $('#huaizhu_ui_font_custom_input').css('display', 'none').val('');
    } else if (fontPresetValues.includes(uiThemeSettings.fontFamily)) {
      $('#huaizhu_ui_font_select').val(uiThemeSettings.fontFamily);
      $('#huaizhu_ui_font_custom_input').css('display', 'none').val('');
    } else {
      $('#huaizhu_ui_font_select').val('__custom__');
      $('#huaizhu_ui_font_custom_input').css('display', 'block').val(uiThemeSettings.fontFamily);
    }

    $('#huaizhu_ui_custom_css_textarea').val(uiThemeSettings.customCss);
  }

  // 三个模式按钮
  $(document).on('click', '.huaizhu-uitheme-btn', function () {
    const mode = $(this).data('uitheme');
    uiThemeSettings.mode = mode;
    saveUiThemeSettings(uiThemeSettings);

    $('.huaizhu-uitheme-btn').removeClass('huaizhu-style-active');
    $(this).addClass('huaizhu-style-active');
    $('#huaizhu_ui_custom_controls').css('display', mode === 'custom' ? 'block' : 'none');

    applyUiTheme();
    setTimeout(() => positionPanelNearBall(modalBox), 0);
  });

  // 自定义模式下的颜色选择器（圆形色盘）和文字输入框互相同步
  function bindUiColorPickerPair(pickerId, textId, settingKey) {
    $('#' + pickerId).on('input', function () {
      const val = $(this).val();
      $('#' + textId).val(val);
      uiThemeSettings[settingKey] = val;
      saveUiThemeSettings(uiThemeSettings);
      applyUiTheme();
    });
    $('#' + textId).on('change', function () {
      ...
      if (!/^#[0-9a-fA-F]{6}$/.test(val)) {
        $(this).val(uiThemeSettings[settingKey]); // 不是合法hex就还原，避免存进脏数据
        return;
      }
      $('#' + pickerId).val(val);
      uiThemeSettings[settingKey] = val;
  ...
  bindUiColorPickerPair('huaizhu_ui_bg_picker', 'huaizhu_ui_bg_text', 'bgColor');
  bindUiColorPickerPair('huaizhu_ui_text_picker', 'huaizhu_ui_text_text', 'textColor');
  bindUiColorPickerPair('huaizhu_ui_accent_picker', 'huaizhu_ui_accent_text', 'accentColor');

  $('#huaizhu_ui_radius_slider').on('input', function () {
    const val = parseInt($(this).val());
    uiThemeSettings.radius = val;
    $('#huaizhu_ui_radius_value').text(val + 'px');
    saveUiThemeSettings(uiThemeSettings);
    applyUiTheme();
  });

  $('#huaizhu_ui_fontscale_slider').on('input', function () {
    const val = parseInt($(this).val());
    uiThemeSettings.fontScale = val;
    $('#huaizhu_ui_fontscale_value').text(val + '%');
    saveUiThemeSettings(uiThemeSettings);
    applyUiTheme();
  });

  // 字体选择：选"自定义字体名称…"才展开下面的文字输入框，否则直接用下拉框的值
  $('#huaizhu_ui_font_select').on('change', function () {
    const val = $(this).val();
    if (val === '__custom__') {
      $('#huaizhu_ui_font_custom_input').css('display', 'block').trigger('focus');
      // 这里先不立刻保存，等用户在下面文本框里实际输入内容后再生效，避免一进自定义就把fontFamily设成无效的"__custom__"字符串
    } else {
      $('#huaizhu_ui_font_custom_input').css('display', 'none');
      uiThemeSettings.fontFamily = val;
      saveUiThemeSettings(uiThemeSettings);
      applyUiTheme();
    }
  });

  $('#huaizhu_ui_font_custom_input').on('input', function () {
    uiThemeSettings.fontFamily = $(this).val().trim();
    saveUiThemeSettings(uiThemeSettings);
    applyUiTheme();
  });

  // 高级"自定义CSS代码"折叠区域的展开/收起
  $('#huaizhu_advanced_css_toggle').on('click', function () {
    $('#huaizhu_advanced_css_content').slideToggle(150);
    $(this).find('.huaizhu-collapse-arrow').toggleClass('huaizhu-collapsed');
    setTimeout(() => positionPanelNearBall(modalBox), 160);
  });

  $('#huaizhu_ui_custom_css_textarea').on('input', function () {
    uiThemeSettings.customCss = $(this).val();
    saveUiThemeSettings(uiThemeSettings);
    applyUiTheme();
  });
  ...
  $('.huaizhu-tab-btn').on('click', function () {
    ...
    if (tabName === 'warehouse') {
      $('#huaizhu_favorite_edit_view').css('display', 'none');
      $('#huaizhu_warehouse_list_view').css('display', 'block');
      renderRecentList();
      renderFavoriteList();
    }

    if (tabName === 'template') {
      $('#huaizhu_template_edit_view').css('display', 'none');
      $('#huaizhu_template_list_view').css('display', 'block');
      refreshGroupFilterOptions();
      renderTemplateList();
    }

    if (tabName === 'beautify') {
      loadUiThemeSettingsIntoForm();
    }
    ...
    if (tabName === 'appsettings') {
      loadAppSettingsIntoForm();
    }
  ...
  const HUAIZHU_APP_SETTINGS_KEY = 'huaizhu_app_settings';

  function loadAppSettings() {
    try {
      const raw = localStorage.getItem(HUAIZHU_APP_SETTINGS_KEY);
      const parsed = raw ? JSON.parse(raw) : {};
      return {
        detectDuplicate: parsed.detectDuplicate !== undefined ? !!parsed.detectDuplicate : true,
      };
    } catch (e) {
      return { detectDuplicate: true };
    }
  }

  function saveAppSettings(settings) {
    localStorage.setItem(HUAIZHU_APP_SETTINGS_KEY, JSON.stringify(settings));
  }

  let appSettings = loadAppSettings();

  function loadAppSettingsIntoForm() {
    $('#huaizhu_setting_detect_duplicate').prop('checked', appSettings.detectDuplicate);
  }

  $('#huaizhu_setting_detect_duplicate').on('change', function () {
    appSettings.detectDuplicate = this.checked;
  ...
  const HUAIZHU_ALL_KEYS = [
    'huaizhu_recent_list',
    'huaizhu_favorite_list',
    'huaizhu_template_list',
    'huaizhu_preset_list',
    'huaizhu_api_config',
    'huaizhu_app_settings',
    'huaizhu_beautify_custom_style',
    'huaizhu_ui_mode_settings',
    'huaizhu_result_zoom_preferred',
    'huaizhu_entry_zoom_preferred',
    'huaizhu_perf_mode_settings',
    'huaizhu_token_settings',
  ];

  // 查看数据状态
  $('#huaizhu_data_check_btn').on('click', () => {
    const result = $('#huaizhu_data_check_result');
    const btn = $('#huaizhu_data_check_btn');
    ...
    let html = '';
    const labels = {
      'huaizhu_recent_list': '最近生成',
      'huaizhu_favorite_list': '我的收藏',
      'huaizhu_template_list': '随机生成',
      'huaizhu_preset_list': '预设',
      'huaizhu_api_config': '独立API设置',
    };
    for (const [key, label] of Object.entries(labels)) {
      try {
        const raw = localStorage.getItem(key);
        if (!raw) {
          html += label + '：<span style="color:#f66">无数据（null）</span><br>';
        } else {
          const parsed = JSON.parse(raw);
          const count = Array.isArray(parsed) ? parsed.length + ' 条' : '有配置';
          html += label + '：<span style="color:#6f6">' + count + '</span><br>';
        }
      } catch (e) {
        html += label + '：<span style="color:#f66">读取出错</span><br>';
      }
    }
    result.html(html).slideDown(150);
    btn.text('🔼 点击收起');

    // 展开后把面板内容滚到顶部，确保"收起"按钮不被推到屏幕外看不到
    setTimeout(() => {
      positionPanelNearBall(modalBox);
  ...
  // 导出全部数据
  $('#huaizhu_data_export_btn').on('click', () => {
    try {
      const backup = {};
      HUAIZHU_ALL_KEYS.forEach((key) => {
      ...
      a.href = url;
      a.download = '槐序戏台备份_' + new Date().toLocaleDateString().replace(/\//g, '-') + '.json';
  ...
  // 显示/隐藏导入区域
  $('#huaizhu_data_import_btn').on('click', () => {
    $('#huaizhu_data_import_file').val('').trigger('click');
  });

  $('#huaizhu_data_import_file').on('change', function () {
    ...
    reader.onload = (e) => {
      try {
        ...
        alert('已成功恢复 ' + count + ' 项数据，请刷新页面让插件重新加载。');
      } catch (err) {
  ...
  // ===================================================================
  // 相似度检测：保存前扫描是否已有名称相近的条目，给出提示（不拦截，由用户决定是否继续）
  // 使用简单的"去掉标点和空格后的包含关系"判断，不追求精确算法，实用够用即可
  // ===================================================================
  ...
  function isSimilarName(nameA, nameB) {
    ...
    if (!a || !b) return false;
    // 包含关系（其中一个是另一个的子串）或者编辑距离很近（长度差不超过2且包含一方长度的60%以上的公共前缀）
    if (a.includes(b) || b.includes(a)) return true;
    // 简单的前N字符相同
    const minLen = Math.min(a.length, b.length);
    if (minLen >= 4) {
      const commonPrefix = [...a].filter((c, i) => c === b[i]).length;
      if (commonPrefix / minLen >= 0.75) return true;
    }
    return false;
  }

  function findSimilarInList(newName, list, nameField) {
    return list.filter((item) => isSimilarName(newName, item[nameField] || ''));
  }
  ...
  function buildEntryButtonsAndBox(item, extraButtonsHtml) {
    ...
    return `
      ...
      <div class="huaizhu-entry-buttons">
        ${beautifyBtnHtml}
        <button type="button" class="menu_button huaizhu-entry-zoom-btn">🔍 放大</button>
        ${extraButtonsHtml}
      </div>
    `;
  }
  ...
  function buildRecentEntryContent(item) {
    return `
      ...
      ${buildEntryButtonsAndBox(
        item,
        `<button type="button" class="menu_button huaizhu-recent-edit-btn" data-id="${item.id}">编辑</button>
  ...
  function buildFavoriteEntryContent(item) {
    return buildEntryButtonsAndBox(
      item,
      `<button type="button" class="menu_button huaizhu-favorite-edit-btn" data-id="${item.id}">编辑</button>
       <button type="button" class="menu_button huaizhu-copy-btn" data-content="${encodeURIComponent(item.content)}">复制内容</button>
  ...
  $(document).on('click', '.huaizhu-entry-header', function () {
    ...
    if (contentEl.attr('data-loaded') !== '1') {
      ...
      // 新展开的条目沿用上次记住的放大/正常偏好，不用每条都重新点一次
      const zoomed = loadEntryZoomPreferred();
      contentEl.find('.huaizhu-entry-render-box').toggleClass('huaizhu-zoomed', zoomed);
      contentEl.find('.huaizhu-entry-zoom-btn').text(zoomed ? '🔎 缩小' : '🔍 放大');
    }
  ...
  // 仓库条目的"放大/缩小"——跟生成结果窗口的放大是同一个思路（只放大尺寸和字号，不改变定位方式），
  // 这里是全局偏好：点任意一条的放大按钮，当前所有展开着的条目会一起变大/变小，并记住偏好供下次使用
  const HUAIZHU_ENTRY_ZOOM_KEY = 'huaizhu_entry_zoom_preferred';

  function loadEntryZoomPreferred() {
    return localStorage.getItem(HUAIZHU_ENTRY_ZOOM_KEY) === '1';
  }

  function saveEntryZoomPreferred(val) {
    localStorage.setItem(HUAIZHU_ENTRY_ZOOM_KEY, val ? '1' : '0');
  }

  $(document).on('click', '.huaizhu-entry-zoom-btn', function (e) {
    e.stopPropagation(); // 不要触发外层折叠标题的展开/收起

    const newZoomed = !loadEntryZoomPreferred();
    saveEntryZoomPreferred(newZoomed);

    // 同步当前所有已经展开的条目，而不只是被点击的这一个，体验上更一致
    $('.huaizhu-entry-render-box').toggleClass('huaizhu-zoomed', newZoomed);
    $('.huaizhu-entry-zoom-btn').text(newZoomed ? '🔎 缩小' : '🔍 放大');

    setTimeout(() => positionPanelNearBall(modalBox), 160);
  });

  function renderRecentList() {
    const keyword = $('#huaizhu_recent_search_input').val().trim().toLowerCase();
    ...
    if (keyword) {
      list = list.filter((item) => (item.instruction || '').toLowerCase().includes(keyword));
    }

    if (list.length === 0) {
      container.html('<p style="opacity:0.6; font-size:0.85em; padding:8px 0;">' + (keyword ? '没有匹配的记录' : '暂无记录') + '</p>');
      return;
    }
  ...
  $('#huaizhu_recent_search_input').on('input', renderRecentList);
  ...
  function renderFavoriteList() {
    const keyword = $('#huaizhu_favorite_search_input').val().trim().toLowerCase();
    ...
    if (list.length === 0) {
      container.html('<p style="opacity:0.6; font-size:0.85em; padding:8px 0;">' + (keyword ? '没有匹配的收藏' : '暂无收藏') + '</p>');
      return;
    }
  ...
  $('#huaizhu_favorite_search_input').on('input', renderFavoriteList);

  // 新建收藏按钮：不再打开独立弹窗，改成在同一个面板里切到编辑视图
  $('#huaizhu_favorite_add_btn').on('click', () => {
    $('#huaizhu_favorite_name_input').val('');
    $('#huaizhu_favorite_content_input').val('');
    $('#huaizhu_warehouse_list_view').css('display', 'none');
    $('#huaizhu_favorite_edit_view').css('display', 'block');
    setTimeout(() => positionPanelNearBall(modalBox), 0);
  });

  $('#huaizhu_favorite_edit_back, #huaizhu_favorite_edit_cancel').on('click', () => {
    $('#huaizhu_favorite_also_save_template').prop('checked', false);
    $('#huaizhu_favorite_template_group_row').hide();
    $('#huaizhu_favorite_duplicate_warning').hide().html('');
    $('#huaizhu_favorite_edit_save').removeData('force');
    $('#huaizhu_favorite_edit_view').css('display', 'none');
    $('#huaizhu_warehouse_list_view').css('display', 'block');
    setTimeout(() => positionPanelNearBall(modalBox), 0);
  });

  // 勾选"同时保存到随机生成"时展开分组输入框
  $('#huaizhu_favorite_also_save_template').on('change', function () {
    $('#huaizhu_favorite_template_group_row').toggle(this.checked);
    setTimeout(() => positionPanelNearBall(modalBox), 160);
  });
  ...
  function doSaveFavorite() {
    ...
    if (!content) {
      $('#huaizhu_favorite_duplicate_warning')
        .html('⚠ 请填写收藏内容').show();
      return;
    }

    const list = loadFavoriteList();
    list.unshift({ id: Date.now(), name: name || '（无标题）', content });
    saveFavoriteList(list);

    if ($('#huaizhu_favorite_also_save_template').prop('checked')) {
      const groupVal = $('#huaizhu_favorite_template_group_input').val().trim();
      const templateList = loadTemplateList();
      templateList.unshift({ id: Date.now() + 1, name: name || '（无标题）', group: groupVal, content });
      saveTemplateList(templateList);
    }

    $('#huaizhu_favorite_also_save_template').prop('checked', false);
    $('#huaizhu_favorite_template_group_row').hide();
    $('#huaizhu_favorite_template_group_input').val('');
    $('#huaizhu_favorite_duplicate_warning').hide().html('');
    $('#huaizhu_favorite_edit_save').removeData('force');
    $('#huaizhu_favorite_edit_view').css('display', 'none');
    $('#huaizhu_warehouse_list_view').css('display', 'block');
  ...
  $('#huaizhu_favorite_edit_save').on('click', () => {
    const name = $('#huaizhu_favorite_name_input').val().trim();
    const saveBtn = $('#huaizhu_favorite_edit_save');

    // 第二次点击（已显示过警告）直接强制保存
    if (saveBtn.data('force')) {
      doSaveFavorite();
      return;
    }
    ...
    if (s.detectDuplicate && name) {
      const similar = [
        ...findSimilarInList(name, loadFavoriteList(), 'name').map(it => '收藏「' + it.name + '」'),
        ...findSimilarInList(name, loadTemplateList(), 'name').map(it => '随机生成「' + it.name + '」'),
      ];
      if (similar.length > 0) {
        $('#huaizhu_favorite_duplicate_warning')
          .html('⚠ 已有相似名称：' + similar.join('、') + '<br><span style="opacity:0.7;font-size:0.9em;">再点一次「保存」强制保存，或修改名称后重试</span>')
          .show();
        saveBtn.data('force', true);
        setTimeout(() => positionPanelNearBall(modalBox), 0);
        return;
      }
    }
  ...
  // ===================================================================
  // 仓库条目编辑弹窗：最近生成和收藏共用，通过在弹窗上设置data-type和data-id来区分当前编辑哪条
  // ===================================================================

  let entryEditCurrentType = null;
  let entryEditCurrentId = null;

  function openEntryEditDialog(type, id) {
    const item = findEntryItem(type, id);
    if (!item) return;

    entryEditCurrentType = type;
    entryEditCurrentId = id;

    const titleText = type === 'recent' ? '编辑（最近生成）' : '编辑（我的收藏）';
    $('#huaizhu_entry_edit_title').text(titleText);
    // 最近生成的"名称"存的是instruction字段；收藏的存name字段
    const nameVal = type === 'recent' ? (item.instruction || '') : (item.name || '');
    $('#huaizhu_entry_edit_name_input').val(nameVal);
    $('#huaizhu_entry_edit_content_input').val(item.content || '');

    const overlay = $('#huaizhu_entry_edit_overlay');
    overlay.css({ display: 'flex', background: 'transparent', alignItems: 'flex-start', justifyContent: 'flex-start' });
    const box = overlay.find('.huaizhu-modal-box')[0];
  ...
  $(document).on('click', '.huaizhu-recent-edit-btn', function (e) {
    e.stopPropagation();
    openEntryEditDialog('recent', parseInt($(this).data('id')));
  });

  $(document).on('click', '.huaizhu-favorite-edit-btn', function (e) {
    e.stopPropagation();
    openEntryEditDialog('favorite', parseInt($(this).data('id')));
  });

  $('#huaizhu_entry_edit_close, #huaizhu_entry_edit_cancel').on('click', () => {
    $('#huaizhu_entry_edit_overlay').css('display', 'none');
    entryEditCurrentType = null;
    entryEditCurrentId = null;
  });

  $('#huaizhu_entry_edit_save').on('click', () => {
    if (!entryEditCurrentType || !entryEditCurrentId) return;

    const newName = $('#huaizhu_entry_edit_name_input').val().trim();
    const newContent = $('#huaizhu_entry_edit_content_input').val().trim();

    if (!newContent) { alert('内容不能为空'); return; }

    if (entryEditCurrentType === 'recent') {
      const list = loadRecentList();
      const item = list.find((it) => it.id === entryEditCurrentId);
      if (item) {
        if (newName) item.instruction = newName;
        item.content = newContent;
        saveRecentList(list);
        renderRecentList();
      }
    } else if (entryEditCurrentType === 'favorite') {
      const list = loadFavoriteList();
      const item = list.find((it) => it.id === entryEditCurrentId);
      if (item) {
    ...
    // 编辑后条目的data-loaded需要清空，下次展开才会拿到更新后的内容重新构建DOM
    $(`.huaizhu-entry-item[data-id="${entryEditCurrentId}"] .huaizhu-entry-content`).attr('data-loaded', '0').slideUp(0);
    $(`.huaizhu-entry-item[data-id="${entryEditCurrentId}"] .huaizhu-collapse-arrow`).addClass('huaizhu-collapsed');
  ...
  // 把分组字段拆成多个分组——统一用"/"分隔（半角/全角都认），一条随机生成可以同时归入多个分组
  function splitGroups(groupStr) {
    if (!groupStr || !groupStr.trim()) return ['未分组'];
    const parts = groupStr.split(/[/／]/).map((g) => g.trim()).filter((g) => g.length > 0);
    return parts.length > 0 ? parts : ['未分组'];
  }
  ...
  function refreshGroupFilterOptions() {
    ...
    list.forEach((item) => {
      splitGroups(item.group).forEach((g) => groups.add(g));
    });
  ...
  function renderTemplateList() {
    ...
    if (groupFilter && groupFilter !== '__all__') {
      list = list.filter((item) => splitGroups(item.group).includes(groupFilter));
    }
    ...
    if (list.length === 0) {
      container.html('<p style="opacity:0.6; font-size:0.85em; padding:8px 0;">暂无匹配的随机生成</p>');
      return;
    }
    ...
    list.forEach((item) => {
      const safeName = $('<div>').text(item.name).html();
      const groupTagsHtml = splitGroups(item.group)
        .map((g) => '<span class="huaizhu-template-group-tag">' + $('<div>').text(g).html() + '</span>')
        .join('');
      html += `
        <div class="huaizhu-entry-item" data-entry-type="template" data-id="${item.id}">
          <div class="huaizhu-entry-header huaizhu-collapsible-title">
            <span class="huaizhu-entry-name">${safeName}</span>
            <span class="huaizhu-template-group-tags">${groupTagsHtml}</span>
            <span class="huaizhu-collapse-arrow huaizhu-collapsed">▾</span>
          </div>
          <div class="huaizhu-entry-content" data-loaded="0" style="display:none;"></div>
        </div>
  ...
  $(document).on('click', '.huaizhu-template-edit-btn', function (e) {
    ...
    $('#huaizhu_template_edit_title').text('编辑随机生成');
  ...
  $('#huaizhu_template_add_btn').on('click', () => {
    ...
    $('#huaizhu_template_edit_title').text('新建随机生成');
  ...
  function openTemplateEditPanel() {
    $('#huaizhu_template_list_view').css('display', 'none');
    $('#huaizhu_template_edit_view').css('display', 'block');
    setTimeout(() => positionPanelNearBall(modalBox), 0);
  }

  function closeTemplateEditPanel() {
    $('#huaizhu_template_edit_view').css('display', 'none');
    $('#huaizhu_template_list_view').css('display', 'block');
    setTimeout(() => positionPanelNearBall(modalBox), 0);
  }

  $('#huaizhu_template_edit_back, #huaizhu_template_edit_cancel').on('click', () => {
    closeTemplateEditPanel();
  });
    ...
    list.forEach((item, idx) => {
      ...
      html += `
        <div class="huaizhu-preset-item">
          ...
          <button type="button" class="huaizhu-icon-btn huaizhu-preset-up-btn" data-id="${item.id}" ${isFirst ? 'disabled' : ''} title="上移">↑</button>
          <button type="button" class="huaizhu-icon-btn huaizhu-preset-down-btn" data-id="${item.id}" ${isLast ? 'disabled' : ''} title="下移">↓</button>
  ...
  // 预设条目上移/下移
  $(document).on('click', '.huaizhu-preset-up-btn', function () {
    const id = parseInt($(this).data('id'));
    const list = loadPresetList();
  ...
  $(document).on('click', '.huaizhu-preset-down-btn', function () {
    const id = parseInt($(this).data('id'));
    const list = loadPresetList();
    const idx = list.findIndex((p) => p.id === id);
  ...
  // 结果窗口：保存到仓库
  // 注意：完全不使用 confirm()/prompt()，移动端WebView会拦截这两个，直接返回false/null导致功能失效
  $('#huaizhu_result_save_to_recent_btn').on('click', () => {
    ...
    if (currentResultRecordId) {
      ...
      if (idx !== -1) { list[idx].content = currentResultText; saveRecentList(list); }
    } else {
      // 第一次保存
    ...
    btn.text('已保存！');
    setTimeout(() => btn.text('保存到仓库'), 1500);
  ...
  function renderIntoShadowContainer(containerEl, htmlContent) {
    ...
    baseStyle.textContent = `
      ...
      /* 兜底约束：就算AI生成的CSS覆盖了上面的样式，这里用!important保证一些最基本的可读性底线 */
      * { letter-spacing: normal !important; word-spacing: normal !important; }
      /* 防止AI用大量position:fixed/absolute的容器堆叠导致布局混乱，改为relative让它们正常参与文档流 */
  ...
  $('#huaizhu_modal_confirm').on('click', async () => {
    ...
    try {
      ...
      // 美化质量强制约束：针对实际使用中出现的三类高频问题加上硬性限制——
      // ①字符被逐个换行（letter-spacing过大/word-break不当导致每字独占一行）
      // ②多个容器叠在一起布局错乱（用了fixed/absolute定位又没限制）
      // ③背景色和文字色太接近、对比度不足导致几乎看不见字
      const styleQualityRule =
        '\n【美化质量强制要求，必须严格遵守，违反则视为输出无效】：' +
        '①文字必须能正常横向阅读，letter-spacing不超过0.1em，word-spacing不超过0.2em，word-break不得设为break-all，line-height在1.4～2.0之间，绝对禁止出现每个字符独占一行的情况；' +
        '②背景色和正文文字颜色必须有充足的对比度（对比度≥4.5:1），例如深色背景必须配亮色文字，浅色背景必须配深色文字，不允许同色系导致文字几乎不可见；' +
        '③不要使用position:fixed或position:absolute来堆叠多个内容容器，所有内容放在一个正常文档流的主容器里即可，不要制造出多个容器叠在一起导致布局混乱的情况；' +
        '④整体排版应符合正常阅读习惯，不要因为追求视觉特效而牺牲基本的可读性。\n';
      ...
      if (wantStyle) {
        ...
        // 不管有没有填写具体风格，只要选了"需要"美化，都强制追加这套质量约束
        styleRule += styleQualityRule;
      } else {
